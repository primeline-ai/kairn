...
**[Stage 5 - Phase 2 GATE: PASS, 2026-07-08]**
- Built via TDD (3 red-green cycles for `redact()` / `render_preview()` / `confirm_import()`, then a self-review cycle + an RC-gate fix cycle, each test-first): `src/kairn/importers/redact.py`. `redact()` = 13 rules (AWS access-key IDs, OpenAI incl. `sk-proj-`, Anthropic, GitHub classic+fine-grained, Slack `xox[baprse]`/`xapp`, Google API key + bare `ya29.` OAuth, Stripe, JWT, PEM private-key blocks incl. a truncated-block fallback, Authorization/Bearer headers, `key=value` secret assignments, and URI-embedded basic-auth credentials), global longest-match-first overlap resolution with a specificity tie-break, idempotent by construction. `render_preview()` = the dry-run "what would be stored" surface, always post-redaction (redacts the `source` field too). `confirm_import()` = fail-safe confirmation gate (no silent first-run writes). **67 tests** (`tests/test_importers_redact.py` + `tests/redaction_corpus.py`), all green; `ruff check`/`format` clean.
- Corpus is externally-sourced per the Stage 3 fix: `tests/redaction_corpus.py` incorporates gitleaks-derived canonical reference fixtures (AWS `AKIAIOSFODNN7EXAMPLE`, the jwt.io sample, GitHub PAT / Slack / Google / Stripe shapes) **as data, not a dependency**, alongside synthetic cases. Gate = **zero missed matches** over that corpus (each case asserts the exact secret substring is absent from the output) PLUS a prose precision guard (ordinary text mentioning password/secret/token/Authorization/AKIA/bearer left byte-for-byte unchanged).
- EPT (real trigger/effect/consumer): a 27-secret realistic blob -> **0 survived**, prose/context intact; `render_preview` verified to contain zero raw secrets (including in `source`); `confirm_import` fail-safe (dry-run or blank -> no write, explicit `yes`/`assume_yes` -> write, `prompt_fn` never called when it must not be); and `redact()` run over a **real 7.2MB Claude Code transcript** completed clean, deterministic, and idempotent (2nd-pass findings = 0).
- PERF: the real-transcript EPT caught a quadratic-backtracking bug in the assignment rule (leading greedy prefix before the keyword alternation; **5876ms -> 1466ms** after removing it). ~6.6 min extrapolated for the measured 2.1GB corpus, under Phase 4's 10-min budget (`--since` windowing remains the documented backstop). A ReDoS regression guard (`test_jwt_pattern_has_no_catastrophic_backtracking`, pathological input asserted <1s) locks in linear behavior and caught **two self-inflicted O(n^2) regressions** during the fix cycle (a possessive-quantifier "hardening" that was ~10000x slower; an unbounded URL-scheme prefix under `(?i)`).
- Independent RC-gate (`feature-dev:code-reviewer`, sonnet/high, background agent) on commit `2e339ac` returned 8 findings; resolved in `9b4fbe3`. **FIXED 5**: URL-embedded basic-auth credentials (new `url-credential` rule, scheme bounded `{0,30}`); Slack `xapp-`/`xoxe-`; OpenAI `sk-proj-` body tail-leak; bare `ya29.` tokens; truncated PEM blocks (line-anchored body, never eats following prose); `render_preview` `source` redaction. **RECONCILED 2** already fixed by my earlier self-review (`sk-proj-`, `xapp-`). **VERIFIED-AND-DECLINED 2 with reasoning** (not blindly implemented): (a) JWT "ReDoS" measured LINEAR on the reviewer's own worst case - a possessive hardening was measured ~10000x slower and reverted, guarded by the regression test; (b) a bare high-entropy secret with no keyword/prefix/URI context = out-of-scope-by-design (see recall limits below).
- POLICY (Phase 2 deliverable): the authoritative redaction policy (default = MASK in place, not drop; PEM blocks masked whole; only the credential masked in assignments/headers/URLs; precision treated as a co-equal property to zero-miss) is documented in the `redact.py` module docstring + this gate block. The user-facing README privacy section is **Phase 4's** gate ("README ... covering ... the privacy model"), so it is deliberately deferred there - writing it now would duplicate Phase 4 and overpromise an unshipped `kairn import claude-code` CLI.
- DOCUMENTED RECALL LIMIT (deliberate, per the Risk section's "bounded by the rule set's recall"): a bare high-entropy secret with NO keyword/prefix/URI context (e.g. a naked 40-char base64 AWS secret key sitting alone in prose) is NOT caught - entropy-only detection would over-redact the base64 that saturates real transcripts (images, hashes, ids), gutting the precision guard. Such secrets ARE caught when they appear in an assignment, a URL, or with a recognized vendor prefix.
- Commits: `2e339ac` (feature) + `9b4fbe3` (RC-gate + self-review fixes) + this plan update, on `feat/wow9-phase2-redaction`. Per the plan's Verification section, the branch is merged only after CI (`.github/workflows/ci.yml`, Python 3.11/3.12/3.13) is green - PR/CI/merge status recorded in the session closeout + Kairn ship node.
